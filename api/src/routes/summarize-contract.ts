...
import { SYSTEM_CONTRACT_PROMPT } from '../ai/prompts';
import { summarizeContractSchema } from '../ai/schemas';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Block documents larger than ~5MB
  ...
  try {
    ...
    if (!validation.success) {
      console.error('[summarize-contract] Validation failed:', validation.error.format());
    ...
    return sendError(res, 500, err.message || 'Không thể trích xuất thông tin hợp đồng tự động. Vui lòng tự nhập thủ công.');
  }
}
