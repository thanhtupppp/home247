...
import { SYSTEM_AGENT_PROMPT } from '../ai/prompts';
import { agentTools, executeTool } from '../ai/tools';
import { getLandlordBuildings, getLandlordRooms } from '../firestore/queries';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Validate history to block structural injections
    const validation = historySchema.safeParse(history);
    if (!validation.success) {
      console.error('[run-agent] History validation failed:', validation.error.format());
      return sendError(res, 400, 'Lịch sử cuộc hội thoại không đúng định dạng hợp lệ.');
    }
    const cleanHistory = validation.data;

    // Build system context with landlord's building/room stats
    ...
    if (responseMsg.tool_calls && responseMsg.tool_calls.length > 0) {
      ...
      // Second call to interpret tool results
